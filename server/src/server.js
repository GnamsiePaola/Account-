...
app.get('/api/clients', async (req, res) => {
    ...
    try {
        ...
        if (businessId) {
            query += ' WHERE Business_idBusiness = ?';
...
app.post('/api/transactions', async (req, res) => {
    const { type, category, amount, description, date, business_id } = req.body;
    try {
        const [result] = await pool.promise().query(
            'INSERT INTO transactions (type, category, amount, description, date, business_id) VALUES (?, ?, ?, ?, ?, ?)',
            [type, category, amount, description, date, business_id]
        );
...
app.put('/api/transactions/:id', async (req, res) => {
    ...
    try {
        const [result] = await pool.promise().query(
            'UPDATE transactions SET type = ?, category = ?, amount = ?, description = ?, date = ?, business_id = ? WHERE id = ?',
            [type, category, amount, description, date, business_id, id]
        );
...
app.post('/api/inventory', async (req, res) => {
    const { name, category, quantity, unitCost, description, business_id } = req.body;
    try {
        const [result] = await pool.promise().query(
            'INSERT INTO inventory (name, category, quantity, unit_cost, description, business_id) VALUES (?, ?, ?, ?, ?, ?)',
            [name, category, quantity, unitCost, description, business_id]
        );
        res.status(201).json({ id: result.insertId, ...req.body });
    } catch (error) {
...
app.put('/api/inventory/:id', async (req, res) => {
    ...
    try {
        const [result] = await pool.promise().query(
            'UPDATE inventory SET name = ?, category = ?, quantity = ?, unit_cost = ?, description = ?, business_id = ? WHERE id = ?',
            [name, category, quantity, unitCost, description, business_id, id]
        );
        if (result.affectedRows === 0) {
...
app.get('/api/analytics/summary', async (req, res) => {
    ...
    try {
        ...
        const totalExpenses = parseFloat(rows[0].totalExpenses || 0);
